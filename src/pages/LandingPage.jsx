...
const LiveAuctionPage = ({ onBack }) => {
  ...
  const [soldOverlay, setSoldOverlay] = useState(null); // { type: 'sold'|'unsold', playerName: '', teamName: '', price: '' }
  ...
  // Handle SOLD with full screen overlay
  const handleSold = async () => {
    ...
    // Clear overlay and move to next player after 2 seconds
    setTimeout(() => {
      setSoldOverlay(null);
      moveToNextPlayer();
      setLoading(false);
  ...
  // Handle UNSOLD with full screen overlay
  const handleUnsold = async () => {
    ...
    // Show full screen UNSOLD overlay
    setSoldOverlay({
      type: 'unsold',
      playerName: currentPlayer.name,
      teamName: '',
  ...
  return (
    <div className="live-auction-page auction-mode">
      {/* ============================================ */}
      {/* FULL SCREEN SOLD/UNSOLD OVERLAY               */}
      {/* ============================================ */}
      {soldOverlay && (
        <>
          ...
          <div className="sold-subtitle">
            {soldOverlay.type === 'sold' 
              ? `${soldOverlay.playerName} → ${soldOverlay.teamName} for ${soldOverlay.price}`
              : `${soldOverlay.playerName} goes unsold`
            }
          </div>
        </>
      )}
          ...
          <div className="player-panel">
            {currentPlayer && (
              <>
                ...
                <div className="player-image-centered">
                ...
                <div className="player-details-centered">
